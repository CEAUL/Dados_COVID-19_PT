...
useVars <- c("Date", "Cases_7_Day_Mean", "Cases", "Active",  "Deaths_7_Day_Mean", "Deaths")
cvwd <- rDat[, `:=`(data2 = as.Date(data, format = "%d-%m-%Y"),
  ...
  , `:=`(Date = format(data2, "%a %d %b %Y"),
         ...
         Deaths_7_Day_Mean = sprintf("%4.1f", obitos_Avg7),
...
cvwd[, tail(.SD, 10)][
  ...
  kable(align = c("rrrrrr"), 
        ...
        col.names = c("Date", "Cases 7 Day Mean", "Total Cases", "Active Cases",  "Deaths 7 Day Mean", "Deaths"),
...
cvwd %>% 
  ...
  scale_y_continuous(breaks = seq(0, max(cvwd[, confirmados_novos]), 500)) +
  labs(
